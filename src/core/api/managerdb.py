...
class management_env(baseview.SuperUserpermissions):
    '''

    :argument 环境管理页面api 接口
    ...
    def get(self, request, args=None):
        ...
        try:
            ...
            try:
                ...
                page_number = Env.objects.count()
                info = Env.objects.all().order_by('id')[start:end]
                serializers = Envlist(info, many=True)
                return Response(
                    {
                        'page': page_number,
                        'data': serializers.data,
...
class management_service(baseview.SuperUserpermissions):
    '''
    ...
    def get(self, request, args=None):
        ...
        try:
            ...
            try:
                ...
                if con['valve']:
                    page_number = Service.objects.filter(service_name__contains=con['service_name'],
                                                              env_name__contains=con['env_name']).count()
                    info = Service.objects.filter(service_name__contains=con['service_name'],
                                                       env_name__contains=con['env_name'])[start:end]
                else:
                    page_number = Service.objects.count()
