      ...
      <Col span="24">
        <Alert type="warning" show-icon>
          ...
          <template slot="desc">
            ...
            <br>2.由于各个邮件服务提供商对于垃圾邮件过滤的机制各不相同，可能会造成邮件无法接收的情况。所以使用前请测试是否稳定,推荐使用搜狐
            <br>3.只有开启相应的消息推送开关后，消息推送才会开启。否则只有站内信一种消息推送方式。
            <br>4.设置最大Limit数后，所有的查询语句的查询结果都不会超过这个数值。
            <br>5.开启多级审核开关后,用户组将新增执行人角色，只有执行人角色的用户才能最终执行工单。关闭后执行人角色用户将全部更改为使用者
            <br>6.查询审核开关开启后，所有的查询都必须通过管理员同意才能进行。关闭则可自主查询
            <br>7.设置脱敏字段后，查询时如匹配到对应字段则该字段将只会以******显示
          </template>
        </Alert>
        <Card>
          <p slot="title">
            ...
            <Col span="12">
              <Card>
                ...
                <Form :label-width="120">
                  ...
                  <FormItem label="密码:">
                    <Input
                      placeholder="Inception 密码(如未设置密码则不填写)"
                      type="password"
                      v-model="inception.password"
                    ></Input>
                  </FormItem>
                  ...
                  <FormItem label="备份库密码:">
                    <Input
                      placeholder="备份库 密码(如未设置密码则不填写)"
                      type="password"
                      v-model="inception.back_password"
                    ></Input>
                  </FormItem>
                </Form>
              </Card>
            ...
            <Col span="12">
              ...
              <Alert style="margin-left: 5%" type="warning" show-icon>
                ...
                <template slot="desc">
                  ...
                  <br>2.请正确填写Inception备份库相关信息，否则将无法获得回滚语句。(无法获得回滚语句的原因有多种这只是其中之一)
                  <br>3.LDAP登录用户名，必须全局唯一。
                </template>
              </Alert>
            </Col>
            <Col span="12">
              <Card>
                <p slot="title">Apollo配置</p>
                <Alert type="warning" show-icon>
                  tips:参考
                  <a
                    href="https://github.com/ctripcorp/apollo/wiki/Apollo%E5%BC%80%E6%94%BE%E5%B9%B3%E5%8F%B0"
                  >Apollo开放平台</a>
                </Alert>
                <Form :label-width="120">
                  <FormItem label="服务地址url:">
                    <Input v-model="apollo.url"></Input>
                  </FormItem>
                  <FormItem label="token:">
                    <Input v-model="apollo.token"></Input>
                  </FormItem>
                  <Button type="primary" @click="apollo_test()">ldap测试</Button>
                </Form>
              </Card>
            </Col>
            <Col span="12">
              ...
              <Card style="margin-left: 5%">
                ...
                <Form :label-width="120">
                  ...
                  <FormItem label="排除数据库:">
                    <Tag
                      v-for="v in other.exclued_db_list"
                      :key="v"
                      :name="v"
                      type="border"
                      closable
                      color="blue"
                      @on-close="handleClose_exclued_db"
                    >{{ v }}</Tag>
                    <br>
                  ...
                  <FormItem label="可注册邮箱后缀:">
                    <Tag
                      v-for="v in other.email_suffix_list"
                      :key="v"
                      :name="v"
                      type="border"
                      closable
                      color="blue"
                      @on-close="handleCloseemail"
                    >{{ v }}</Tag>
                    <br>
                  ...
                  <FormItem label="脱敏字段:">
                    <Tag
                      v-for="v in other.sensitive_list"
                      :key="v"
                      :name="v"
                      type="border"
                      closable
                      color="blue"
                      @on-close="handleClose3"
                    >{{ v }}</Tag>
                    <br>
  ...
  methods: {
    ...
    save_upload () {
      axios.post(`${this.$config.url}/setting/save`, {
        ...
        'apollo': JSON.stringify(this.apollo)
      })
        .then(res => {
          this.$config.notice(res.data)
        })
