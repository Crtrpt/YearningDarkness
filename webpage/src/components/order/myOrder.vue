    ...
    <Row>
      <Card>
        ...
        <Form inline ref="queryForm">
          ...
          <Alert type="success">Tips:dev环境自动审核通过,stg环境需要审核人审核</Alert>
        </Form>
        <Row>
          <Col span="24">
            <Table border :columns="columnsName" :data="table_data" stripe size="small">
              <template slot-scope="{ row, index }" slot="action">
                <ButtonGroup v-model="buttonSize" type="button">
                  <Button @click="openOrder(row)" size="small">详细</Button>
                  <Button type="success" @click="switchEnv(row)" size="small">上线</Button>
                  <Button
                    type="text"
                    @click="orderReject(row)"
                    v-if="row.status === 0"
                    size="small"
                  >驳回理由</Button>
                </ButtonGroup>
